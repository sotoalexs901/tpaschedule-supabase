...
import EventRsvpSummary from "../components/EventRsvpSummary.jsx";
      ...
      >
        <div
          style={{
            fontSize: 12,
            fontWeight: 800,
            color: isToday ? "#b45309" : "#1769aa",
          }}
        >
          {person.monthLabel}{" "}
          {formatBirthdayDay(person.birthdayDay)}
        </div>
...
export default function DashboardPage() {
  ...
  const fetchMainMessage = async () => {
    try {
      ...
      console.error(
        "Error loading main dashboard message:",
        err
      );
    }
  };
  ...
  const fetchBirthdays = async () => {
    ...
    try {
      ...
      const items = userSnap.docs
        ...
        .filter(
          (item) =>
            Number(item.birthdayMonth) === month
        )
        .sort((a, b) => {
          const dayCompare =
            Number(a.birthdayDay) -
            Number(b.birthdayDay);
  ...
  const fetchEmployeeOfMonth = async () => {
    ...
    try {
      ...
      console.error(
        "Error loading employee of month:",
        err
      );
  ...
  const fetchBlockedEmployees = async () => {
    ...
    try {
      ...
      console.error(
        "Error loading blocked employees:",
        err
      );
  ...
  const fetchPhotos = async () => {
    ...
    try {
      ...
      console.error(
        "Error loading dashboard photos:",
        err
      );
    ...
    navigate("/messages", {
      state: {
        ...
        recipientName:
          employee.employeeName || "",
        prefilledMessage:
          `Congratulations ${personName}! You were selected as Employee of the Month. Great job and thank you for your hard work!`,
      },
    });
  };
    ...
    >
      <div
        style={{
          ...
          color: "#fff",
          boxShadow:
            "0 12px 28px rgba(15,76,129,0.16)",
          marginBottom: 14,
        }}
      >
        <div
          ...
          <div>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {selectedRecognition.photoURL ||
                    selectedRecognition.profilePhotoURL ? (
                      <img
                        src={
                          selectedRecognition.photoURL ||
                          selectedRecognition.profilePhotoURL
                        ...
                        alt={
                          selectedRecognition.employeeName ||
                          "Employee"
                        }
                        style={{
                          width:
                  ...
                  >
                    ...
                    >
                      {selectedRecognition.position ||
                        "\u2014"}{" "}
                      {"\u00B7"}{" "}
                      {selectedRecognition.department ||
                        "\u2014"}
                    </div>

                    {selectedRecognition.airline && (
                      <div
                        style={{
                          marginTop:
                            3,
            ...
            >
              ...
              <button
                type="button"
                disabled={
                  !selectedRecognition.userId &&
                  !selectedRecognition.username
                }
                onClick={() =>
                  handleMessageEmployeeOfMonth(
                    selectedRecognition
                  )
                }
                style={{

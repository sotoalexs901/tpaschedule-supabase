...
function Metric({ label, value, tone = "default" }) {
  const t = { default: ["#f8fbff", "#dbeafe", "#0f172a"], green: ["#ecfdf5", "#a7f3d0", "#166534"], amber: ["#fff7ed", "#fdba74", "#9a3412"], red: ["#fff1f2", "#fecdd3", "#9f1239"], blue: ["#edf7ff", "#cfe7fb", "#1769aa"], slate: ["#f8fafc", "#e2e8f0", "#334155"] }[tone];
...
function eventLabel(type) { return type === "CODE_24" ? "Code 24 / Bag Return" : type === "CODE_39" ? "Code 39" : type === "EXCEPTION_DELIVERY" ? "Exception Delivery" : "Other"; }
...
function recalc(events) {
  ...
  const code24Created = code24.filter(e => e.code24Created === true || e.code24Created === "Yes").length;
  const code39 = events.filter(e => e.eventType === "CODE_39");
  const exceptions = events.filter(e => e.eventType === "EXCEPTION_DELIVERY");
  return {
    totalEvents: events.length,
    code24BagReturnEvents: code24.length,
    code24CreatedCount: code24Created,
    code24Rate: code24.length ? Number((code24Created / code24.length * 100).toFixed(2)) : 0,
    ...
    exceptionFedExCount: exceptions.filter(e => e.deliveryMethod === "FedEx").length,
    exceptionSddCount: exceptions.filter(e => e.deliveryMethod === "SDD").length,
    exceptionOtherMethodCount: exceptions.filter(e => e.deliveryMethod === "Other").length,
...
export default function BSODailyManagementPage() {
  ...
  const emptyFilters = { startDate: "", endDate: "", shift: "all", eventType: "all", supervisor: "", employee: "", status: "all", faultStation: "", lossCode: "", deliveryMethod: "all", exceptionReason: "all", netTracerFile: "", search: "" };
  ...
  const flat = useMemo(() => reports.flatMap(r => (Array.isArray(r.events) ? r.events : []).map((e, idx) => ({ ...e, eventIndex: idx, reportIdDoc: r.id, reportDate: r.reportDate, shift: r.shift, supervisorName: r.supervisorName || r.submittedByName || "", reportStatus: r.status, reviewStatus: r.reviewStatus, createdAt: r.createdAt, reportNotes: r.notes }))), [reports]);
  ...
  const filtered = useMemo(() => flat.filter(e => {
    ...
    if (filters.deliveryMethod !== "all" && e.deliveryMethod !== filters.deliveryMethod) return false;
    if (filters.exceptionReason !== "all" && e.exceptionReason !== filters.exceptionReason) return false;
    if (filters.netTracerFile && !String(e.netTracerFile || "").toLowerCase().includes(filters.netTracerFile.toLowerCase())) return false;
    const q = filters.search.trim().toLowerCase();
    if (q) {
      const hay = [e.passengerName,e.pnr,e.flightNumber,e.bagTags,e.reportId,e.worldTracerId,e.netTracerFile,e.agentCode,e.deliveryMethod,e.exceptionReason,e.employee,e.supervisorName,e.comments,e.otherDescription].join(" ").toLowerCase();
  ...
  const totals = useMemo(() => {
    ...
    const exceptions = filtered.filter(e => e.eventType === "EXCEPTION_DELIVERY");
    return {
      total: filtered.length,
      code24Events: code24.length,
      code24Created,
      code24Rate: code24.length ? code24Created / code24.length * 100 : 0,
  ...
  const exceptionReasonSummary = useMemo(() => EXCEPTION_REASONS.map(reason => ({ reason, count: filtered.filter(e => e.eventType === "EXCEPTION_DELIVERY" && e.exceptionReason === reason).length })).filter(x => x.count > 0), [filtered]);
  ...
  async function deleteEvent(reportId, eventIndex) {
    ...
    const events = (report.events || []).filter((_, i) => i !== eventIndex).map((e,i) => ({ ...e, sequence: i + 1 }));
    try {
  ...
  function exportCsv() {
    const rows = [["Date","Shift","Type","Supervisor","Employee","Passenger","PNR","Flight","Bag Tags","Code 24 Created","BCC Referral","Code 24 Reason","Code 39 Report ID","Fault Station","Loss Code","Bag Type","Bags Checked","Bags Received","World Tracer ID","NetTracer File","Exception Date","Agent Code","Exception Reason","Delivery Method","Status","Review","Comments"], ...filtered.map(e => [e.reportDate,e.shift,eventLabel(e.eventType),e.supervisorName,e.employee,e.passengerName,e.pnr,e.flightNumber,e.bagTags,e.code24Created === true || e.code24Created === "Yes" ? "YES" : "NO",e.bccReferral === true || e.bccReferral === "Yes" ? "YES" : "NO",e.code24Reason,e.reportId,e.faultStation,e.lossCode,e.bagType,e.bagsChecked,e.bagsReceived,e.worldTracerId,e.netTracerFile,e.exceptionDate,e.agentCode,e.exceptionReason,e.deliveryMethod,e.status || e.reportStatus,e.supervisorReview,e.comments])];
  ...
  return <div style={{ display: "grid", gap: 16, fontFamily: "Poppins, Inter, system-ui, sans-serif" }}>
    <div style={{ background: "linear-gradient(135deg,#0f5c91,#1f7cc1 45%,#6ec6e8)", borderRadius: 22, padding: isMobile ? 15 : 22, color: "#fff" }}><div style={{ fontSize: 10, fontWeight: 900, letterSpacing: ".15em", textTransform: "uppercase", opacity: .8 }}>AEROSTATION HUB | AA BSO</div><h1 style={{ margin: "7px 0 5px", fontSize: isMobile ? 22 : 30 }}>BSO Daily Management</h1><div style={{ fontSize: 13, opacity: .9 }}>KPI dashboard for Code 24, Code 39, Exception Delivery and other BSO office events.</div></div>
    ...
    <Card><div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 14 }}><h2 style={{ margin: 0 }}>Filters</h2><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Button variant="secondary" onClick={exportCsv}>Export CSV</Button><Button variant="dark" onClick={() => window.print()}>Print</Button></div></div>
      <div style={grid}>
        ...
        <div><Label>Quick Search</Label><Input value={filters.search} onChange={e => setFilters(p => ({ ...p, search: e.target.value }))} placeholder="PNR, bag tag, NetTracer, report ID, passenger..." /></div>
      </div>
      <div style={{ marginTop: 12, textAlign: "right" }}><Button variant="secondary" onClick={() => setFilters(emptyFilters)}>Clear Filters</Button></div>
    </Card>

    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,minmax(0,1fr))" : "repeat(10,minmax(0,1fr))", gap: 8 }}>
      <Metric label="Total Events" value={totals.total} tone="blue" />
      <Metric label="Bag Returns" value={totals.code24Events} />
      ...
      <Metric label="Exception Delivery" value={totals.exceptions} tone="blue" />
      <Metric label="FedEx / SDD" value={`${totals.fedEx} / ${totals.sdd}`} tone="slate" />
    ...
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 12 }}><div><h2 style={{ margin: 0 }}>Exception Delivery KPI</h2><div style={{ marginTop: 4, color: "#64748b", fontSize: 12, fontWeight: 700 }}>Delivery method and exception-reason breakdown for the selected filters.</div></div></div>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,minmax(0,1fr))", gap: 9, marginBottom: 12 }}>
        <Metric label="FedEx" value={totals.fedEx} tone="blue" />
        <Metric label="SDD" value={totals.sdd} tone="green" />
        <Metric label="Other Method" value={totals.otherDelivery} tone="slate" />
      </div>
      {exceptionReasonSummary.length === 0 ? <div style={{ padding: 14, background: "#f8fafc", borderRadius: 12, color: "#64748b", fontWeight: 700 }}>No Exception Delivery data for the selected filters.</div> : <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit,minmax(210px,1fr))", gap: 8 }}>{exceptionReasonSummary.map(row => <div key={row.reason} style={{ border: "1px solid #dbeafe", borderRadius: 13, padding: 11, background: "#f8fbff" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 800 }}>{row.reason}</div><div style={{ marginTop: 3, fontSize: 20, fontWeight: 900, color: "#1769aa" }}>{row.count}</div></div>)}</div>}
    </Card>
    ...
    <Card>
      ...
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1500 }}><thead><tr style={{ background: "#f8fbff" }}><th style={th}>Date</th><th style={th}>Type</th><th style={th}>Employee</th><th style={th}>Passenger / PNR</th><th style={th}>Flight</th><th style={th}>Bag Tags</th><th style={th}>Code 24</th><th style={th}>Code 39 ID</th><th style={th}>Exception Delivery</th><th style={th}>Fault / Loss</th><th style={th}>Supervisor</th><th style={th}>Actions</th></tr></thead><tbody>
        {!filtered.length ? <tr><td colSpan={12} style={td}>{loading ? "Loading..." : "No data found."}</td></tr> : filtered.map((e, i) => <tr key={`${e.reportIdDoc}-${e.eventIndex}-${i}`}><td style={td}>{e.reportDate}<br/><span style={{ color: "#64748b" }}>{e.shift}</span></td><td style={td}><b>{eventLabel(e.eventType)}</b></td><td style={td}>{e.employee || "-"}{e.agentCode ? <><br/><span style={{ color: "#64748b" }}>Code: {e.agentCode}</span></> : null}</td><td style={td}>{e.passengerName || "-"}<br/><b>{e.pnr || "-"}</b></td><td style={td}>{e.flightNumber || "-"}</td><td style={td}>{e.bagTags || "-"}</td><td style={td}>{e.eventType === "CODE_24" ? (e.code24Created === true || e.code24Created === "Yes" ? "Created" : "Avoided") : "-"}</td><td style={td}>{e.reportId || "-"}</td><td style={td}>{e.eventType === "EXCEPTION_DELIVERY" ? <><b>{e.netTracerFile || "-"}</b><br/>{e.deliveryMethod || "-"}<br/><span style={{ color: "#64748b" }}>{e.exceptionReason || "-"}</span></> : "-"}</td><td style={td}>{e.faultStation || "-"}<br/>{e.lossCode || "-"}</td><td style={td}>{e.supervisorName || "-"}</td><td style={td}><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><Button variant="secondary" onClick={() => setSelectedId(e.reportIdDoc)}>View</Button><Button variant="danger" disabled={working === `${e.reportIdDoc}-${e.eventIndex}`} onClick={() => deleteEvent(e.reportIdDoc, e.eventIndex)}>Delete Event</Button></div></td></tr>)}
      </tbody></table></div>
    </Card>
    ...
    {selectedReport && <Card style={{ border: "1px solid #bfdbfe" }}>
      ...
      <div style={{ display: "grid", gap: 10 }}>{(selectedReport.events || []).map((e, idx) => <div key={idx} style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 13, background: "#f8fbff" }}><div style={{ fontWeight: 900 }}>{idx + 1}. {eventLabel(e.eventType)}</div><div style={{ marginTop: 6, fontSize: 12.5, lineHeight: 1.7, color: "#334155" }}><b>Employee:</b> {e.employee || "-"} · <b>Passenger:</b> {e.passengerName || "-"} · <b>PNR:</b> {e.pnr || "-"} · <b>Bag Tags:</b> {e.bagTags || "-"}{e.eventType === "CODE_24" && <> · <b>Code 24:</b> {e.code24Created ? "Created" : "Avoided"} · <b>BCC:</b> {e.bccReferral ? "Yes" : "No"}</>}{e.eventType === "CODE_39" && <> · <b>Report ID:</b> {e.reportId || "-"} · <b>Fault:</b> {e.faultStation || "-"} · <b>Loss:</b> {e.lossCode || "-"} · <b>Bags:</b> {e.bagsChecked ?? 0}</>}{e.eventType === "EXCEPTION_DELIVERY" && <> · <b>NetTracer:</b> {e.netTracerFile || "-"} · <b>Date:</b> {e.exceptionDate || "-"} · <b>Agent Code:</b> {e.agentCode || "-"} · <b>Reason:</b> {e.exceptionReason || "-"} · <b>Delivery:</b> {e.deliveryMethod || "-"}</>}<br/><b>Review:</b> {e.supervisorReview || "-"}{e.comments ? <> · <b>Comments:</b> {e.comments}</> : null}</div></div>)}</div>
    </Card>}
  </div>;
}
